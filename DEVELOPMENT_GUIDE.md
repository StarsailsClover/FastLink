...
| **libomnilink** | P2P 全链接 | `src/ice.rs`, `src/stun.rs`, `src/nat.rs` |
| **libfasttransport** | 传输层 | `src/transport.rs` |
| **libfastdht** | 分布式哈希表 | `src/dht.rs` |
| **libantidpi** | 反 DPI | `src/antidpi.rs` |
...
| **fastlink-server** | 服务器中继 | `src/server.rs` |
| **fastlink-swift** | 抗 DPI 隧道 | `src/swift.rs` |
...
- [ ] **实现测试用例** - 9 个测试文件目前是占位符
- [ ] **清理编译警告** - `cargo fix` 可自动修复大部分

### 🟡 中优先级 (本周)

- [ ] **完成 FastLinkMC 子模块** - 决定如何处理未跟踪的目录
- [ ] **完善 CLI 功能** - `apps/fastlink-cli/src/main.rs` 需要扩展
...
- `tools/packet-capture` - 抓包工具待实现
- `tools/benchmarks` - 基准测试待扩展
- `tools/nat-simulator` - NAT 模拟器待实现
...
2. **随机数** - 始终使用 `OsRng` 获取熵
3. **内存安全** - 敏感数据使用 `zeroize` 清理
4. **常量时间** - 比较操作应使用常量时间算法
...
2. **发布说明**: `RELEASE_NOTES_v26.5-20260531.md` - 版本变更
3. **中文说明**: `RELEASE_NOTES_v26.5-20260531_zh.md` - 中文文档
...
### 任务 3: 扩展 CLI (2小时)
添加基础命令到 `apps/fastlink-cli/src/main.rs`:
